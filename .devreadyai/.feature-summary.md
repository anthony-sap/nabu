...
**Feature ID:** user-signup-flow-plan-selection

Complete user signup and plan upgrade flow with workspace invite acceptance:

**Phase 1 - User Sync on First Login:**
- **Automatic user creation** - New Kinde users synced to database on first login
- **Free plan default** - All new users start on Free plan
- **Pending invite detection** - System checks for workspace invites by email

**Phase 2 - Plan Upgrade Flow (No Payment):**
- **Upgrade API** - POST `/api/user/upgrade` with support for Free→Personal, Free→Teams, Personal→Teams
- **Teams workspace creation** - Upgrading to Teams automatically creates a workspace with user as owner
- **Upgrade page** - Plan comparison UI at `/upgrade` showing all plans with features and limits
- **Entitlements refresh** - Frontend hooks automatically refetch after upgrade

**Phase 3 - Workspace Invite Acceptance:**
- **Invite acceptance page** - `/invite/[token]` handles both logged-in and not-logged-in users
- **Email validation** - Ensures logged-in user email matches invite email
- **Role display** - Shows workspace name, role, and permissions before accepting
- **Post-acceptance redirect** - Redirects to notes after successful acceptance

**Phase 4 - Team Member Management:**
- **Members API** - GET/PATCH/DELETE `/api/workspaces/[id]/members` for member management
- **Members page** - `/workspace/[id]/members` with member list, role management, invite UI
- **Invite email sending** - Automated email with invite link when creating invites
- **Pending invites list** - View and manage pending invitations

**Architecture:**
- `lib/user-sync.ts` - User sync service
- `app/api/user/sync/route.ts` - User sync API endpoint
- `app/api/user/upgrade/route.ts` - Plan upgrade API
- `app/(app)/upgrade/page.tsx` - Upgrade page UI
- `app/(auth)/invite/[token]/page.tsx` - Invite acceptance page
- `app/(app)/workspace/[id]/members/page.tsx` - Member management page
- `lib/email/send-invite.ts` - Invite email service
- `lib/email/templates.ts` - Email HTML templates

**Key Features:**
- Seamless Kinde integration with database sync
- Support for all upgrade paths without payment (for testing)
- Team workspace creation on Teams upgrade
- Full invite lifecycle (create, send email, accept, expire)
- Role-based member management (owner, admin, member, guest)

**Files:** 12 created, 3 modified

See [detailed documentation](.devreadyai/completed-features/user-signup-flow-plan-selection_details.md)
